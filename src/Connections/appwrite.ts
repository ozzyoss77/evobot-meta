...
class AppwriteService {
  ...
  async getDocumentByAttribute(databaseId, collectionId, method, attribute, value) {
    try {
      this.logger.log(`🔍 Appwrite: Getting document from ${collectionId} where ${attribute}=${value}`);
      ...
      this.logger.log(`✅ Appwrite: Document retrieved from ${collectionId} for ${attribute}=${value}`);
      return response.data.documents[0];
    } catch (error) {
      this.logger.error(`❌ Appwrite: Error getting document from ${collectionId} - ${error}`);
      return null;
    }
  }
